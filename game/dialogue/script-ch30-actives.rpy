...
label a1:
    python:
        ...
        # Start with the base 5 variants
        num_variants = 5 
        
        # Initialize paths to -1 (meaning "disabled")
        poetry_path_num = -1
        heartbeat_path_num = -1

        # Check if Collaborative Poetry is unlocked (Karma >= 4)
        if karma_lvl() >= 4:
            num_variants += 1
            poetry_path_num = num_variants
            
        # Check if System Heartbeat is available
        if 'sys_info_available' in globals() and sys_info_available:
            num_variants += 1
            heartbeat_path_num = num_variants
            
        # Pick a variant from the available pool
    ...
    if karma_lvl() == 3:
        ...
        if karma_lvl() < 3:
            ...
            elif howAreYouVariant == poetry_path_num and poetry_path_num != -1:
            ...
            elif howAreYouVariant == heartbeat_path_num and heartbeat_path_num != -1:
                call yuri_system_heartbeat
